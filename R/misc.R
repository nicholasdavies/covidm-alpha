...
{
    # do date magic
    dss = lubridate::ymd(date_simulation_start);
    t_ref = as.numeric(dss);
    t_bounds = c(as.numeric(lubridate::ymd(date_measurement_start)) - t_ref,
                 as.numeric(lubridate::ymd(dates_measurement_end)) - t_ref);
