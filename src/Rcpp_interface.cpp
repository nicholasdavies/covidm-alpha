...
//' @export
// [[Rcpp::export]]
Rcpp::List cm_backend_simulate_v2(Rcpp::List parameters, unsigned int n_run = 1, unsigned long int seed = 0, unsigned int n_threads = 1, Rcpp::String r_file_out = "")
{
    ...
    else
    {
        ...
        for (unsigned int r = 0; r < n_run; ++r)
        {
            ...
            Rcpp::Environment data_table = Rcpp::Environment::namespace_env("data.table");
            Rcpp::Function setDT = data_table["setDT"];
            setDT(dynamics_df);
...
//' @export
// [[Rcpp::export]]
Rcpp::DataFrame cm_evaluate_distribution_v2(Rcpp::String dist_code, unsigned int steps = 101, double xmin = 0, double xmax = -1)
{
    Distribution dist(dist_code.get_cstring());
    if (xmax < xmin)
    {
