...
#what do i need
#matrices (prem)
#populations (wpp2019; uk; worldpop)

#the minimum set is prem matrices, wpp2019 demographics.
...
# Load all needed data as data.tables
data(pop)
data(UNlocations)
popF = data.table(popF)
popM = data.table(popM)
UNlocations = data.table(UNlocations)

# Get codes for country-level administrative units
codes = UNlocations[location_type == 4, unique(country_code)];

# Create and tidy population size data
populations = merge(popF[country_code %in% codes, c(1,2,3,18)], popM[country_code %in% codes, c(1,2,3,18)], by = c("country_code", "name", "age"));
names(populations)[4:5] = c("f", "m");
populations[, age := factor(age, levels = limits_to_agegroups(seq(0, 100, by = 5)))];
populations = populations[order(name, age)];

# Assign iso3c codes
populations[, iso3c := countrycode(country_code, origin = "iso3n", dest = "iso3c")];
populations[country_code == 830, iso3c := "XCI"]; # Create custom code for Channel Islands

# Format data
dat_wpp2019 = populations[, .(code = iso3c, name, group = age, f = f * 1000, m = m * 1000)];

# Remove temporary data
rm(pop, popF, popFT, popM, popMT, populations, UNlocations, codes);


# worldpop
# Load data
dat_worldpop = fread("~/Documents/covid-world/demographics/worldpop5yr.lfs.csv");

# Melt and split by sex and age
dat_worldpop = melt(dat_worldpop, id.vars = c("name", "id"), measure.vars = 10:43);
dat_worldpop[, sex := str_sub(variable, 1, 1)];
dat_worldpop[, age := as.numeric(str_sub(variable, 3))];
dat_worldpop = dcast(dat_worldpop, id + name + age ~ sex, value.var = "value");

# Format
names(dat_worldpop) = c("code", "name", "group", "f", "m");
dat_worldpop[, group := paste0(group, "-", as.numeric(group) + 4)];
dat_worldpop[group == "80-84", group := "80+"];
dat_worldpop[, group := factor(group, levels = unique(group))];

# Round to nearest 1
dat_worldpop[, f := round(f)];
dat_worldpop[, m := round(m)];


# uk
# Load males and females count data
ukm = data.table(read_excel("~/Documents/covidm-alpha/data-raw/ukmidyearestimates20192020ladcodes.xls", 7, "A5:CQ431"));
ukf = data.table(read_excel("~/Documents/covidm-alpha/data-raw/ukmidyearestimates20192020ladcodes.xls", 8, "A5:CQ431"));

# Melt and merge
ukm = melt(ukm, id.vars = c("Code", "Name"), measure.vars = 5:95, variable.name = "age", value.name = "m", variable.factor = F);
ukf = melt(ukf, id.vars = c("Code", "Name"), measure.vars = 5:95, variable.name = "age", value.name = "f", variable.factor = F);
uk = merge(ukf, ukm, by = c("Code", "Name", "age"));

# Generate age group in 5 year bands
uk[, age := as.numeric(age)]
uk[is.na(age), age := 90];
uk[, age_group := (age %/% 5) * 5];

# Sum over all individuals within 5 year bands
uk = uk[, .(f = sum(f), m = sum(m)), by = .(code = Code, name = Name, group = age_group)];

# Rename and factor age group
uk[, group := paste0(group, "-", group + 4)];
uk[group == "90-94", group := "90+"];
uk[, group := factor(group, levels = unique(group))];

# Reorder and name
uk = uk[order(name, code, group), ];
dat_uk = uk;
...
# Get contact matrices from two Excel workbooks
matrices = NULL;
...
for (s in sheets1) {
    ...
    matrices = rbind(matrices,
        data.table(name = s,
            context = c("home", "work", "school", "other"),
            matrix = list(mh, mw, ms, mo))
    );
}
...
# Assign iso3c codes
matrices[, code := countrycode(name, origin = "country.name", destination = "iso3c")];

# Format data
dat_prem2017 = matrices[, .(code, name, context, matrix)];

# Remove temporary data
rm(matrices, mh, mo, ms, mw, s, sheets1, sheets2)


usethis::use_data(dat_wpp2019, dat_worldpop, dat_uk, dat_prem2017, internal = TRUE, overwrite = TRUE) # TODO: reintroduce cm_highrisk
...
# usethis::use_data(cm_matrices, overwrite = TRUE)
# usethis::use_data(cm_populations, overwrite = TRUE)
# usethis::use_data(cm_structure_UK, overwrite = TRUE)
# usethis::use_data(cm_highrisk, overwrite = TRUE)

usethis::use_data(cm_matrices, cm_populations, cm_structure_UK, cm_highrisk, internal = TRUE, overwrite = TRUE)
